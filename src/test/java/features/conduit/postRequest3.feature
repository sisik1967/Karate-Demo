...
Feature: Post Request
  ...
  Scenario: Post request
    * def tokenResponse = call read('classpath:features/conduit/postRequest3.feature@login') {"user":{"email":"sabahattin67@gmail.com","password":"qatester2022"}}
   ...
   #* def token2 = tokenResponse.response.user.token #if there is no def statement @login
    ...
    And path 'articles'
    And request {"article":{"title":"test title335","description":"test","body":"karate api test","tagList":""}}
    When method POST
    And status 200
    * print  response